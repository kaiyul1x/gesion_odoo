...
def check_access(record, operation, raise_exception=False):
    try:
        access_right = record.check_access_rights(operation, raise_exception)
        access_rule = record.check_access_rule(operation) == None
...
class GesionDMSApi(http.Controller):
    ...
    def table_data(self, nodes=[], **kwargs):
        ...
        for node in nodes:
            ...
            row_data = node_ins.child_ids.mapped(lambda r: {"model": "gesion_organization_{}".format(r.id),
            ...
            #     _logger.info(users)
            #     user_ids = map(lambda r: r.get("user_id")[0] if r.get("user_id") else False, users)
            ...
            users = request.env['res.users'].search_read(fields=['name', 'description'],
                                                         domain=[('organization_ids', '=', node_ins.id)])
            row_data.extend(map(lambda r: {"model": "res_users_{}".format(r.get('id')),
                                           "type": "human",
                                           "name": r.get('name'),
                                           "desc": r.get('description')}, users))
            ...
            if model == "gesion.organization":
                ...
                node_data = {
                    ...
                    "data": {
                        ...
                        "odoo_parent_id": d.get("parent_id"),
                        "odoo_model": model,
                        "perm_read": check_access(browser, "read", raise_exception=False),
                        "perm_create": check_access(browser, "create", raise_exception=False),
                        "perm_write": check_access(browser, "write", raise_exception=False),
                        "perm_unlink": check_access(browser, "unlink", raise_exception=False)
                    }
                }
    ...
    @http.route(['/gesion_api/task_tree_init_data',
                 '/gesion_api/task_tree_init_data/<string:node_id>',
                 '/gesion_api/task_tree_init_data/<string:node_id>/<int:hierarchy>'
                 ], type="http", auth='public', website=True)
    def task_tree_init_data(self, node_id='#', hierarchy=2, **kwargs):
        if node_id != '#':
            return _('Error tree root!')
        # orm way
        task_data = request.env['project.task'].search_read(
            fields=['name', 'seq', 'parent_task', 'has_child'],
            domain=[('task_level', '<=', hierarchy)],
            order='id')
        fetched_task_data = map(lambda r: {'id': r.get('id'),
                                           'seq': r.get('seq'),
                                           'name': r.get('name'),
                                           'has_child': r.get('has_child'),
                                           'parent_id': r.get('parent_task')[0] if r.get('parent_task') else False
                                           }, task_data)
        parent_d_ids = self.get_parent_node_ids_which_children_loaded({"parent_id": fetched_task_data})
        initial_data = self.node_data_constructor(fetched_task_data, "project.task", True, parent_d_ids)
        return_data = json.dumps(initial_data)
        return request.make_response(return_data, headers=[('Content-Length', len(return_data)),
                                                           ('Content-Type', 'application/json; charset=utf-8')])

    @http.route(['/gesion_api/task_tree_data',
                 '/gesion_api/task_tree_data/<string:node_id>'], type="http", auth='public', website=True)
    def task_tree_data(self, node_id, **kwargs):
        node_num_id = node_id.split('_', 1)[1]
        # orm way
        task_data = request.env['project.task'].search_read(
            fields=['name', 'seq', 'parent_task', 'has_child'],
            domain=[('parent_task', '=', int(node_num_id))],
            order='id')
        fetched_organization_data = map(lambda r: {'id': r.get('id'),
                                                   'seq': r.get('seq'),
                                                   'name': r.get('name'),
        ...
        tree_data = self.node_data_constructor(fetched_organization_data, "project.task", False)
        return_data = json.dumps(tree_data)
        return request.make_response(return_data, headers=[('Content-Length', len(return_data)),
                                                           ('Content-Type', 'application/json; charset=utf-8')])
...
#----------------------
    ...
    def carousel_map_json_data(self, project_id, **kwargs):
        ...
        task_data = []
        task_data.extend(map(lambda r: {"id": r.get('id'),
                                        "sequence": r.get('seq'),
                                        "name": r.get('name'),
                                        ...
                                        "data":{
                                            "designer": r.get('designer')[1] if r.get('designer') else "null",
