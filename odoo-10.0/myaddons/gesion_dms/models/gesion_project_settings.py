...
class TaskStage(models.Model):
    ...
    description = fields.Text(related='stage.description', string='Stage Description')
    # stage_id = fields.Many2one('project.stage', string='Stage')
    stage = fields.Many2one('project.task.type')
    issued = fields.Boolean(string='Current Stage')
    ...
    milestone = fields.Many2one('milestone', string='Mile Stone')
    project_id = fields.Many2one(related='task_id.project_id')
    # project_id = fields.Many2one(related='task_id.project_id')
    # project_id = fields.Many2one('project.task', related='task_id.project_id.id', store=True)

    # 审批
    current_operator = fields.Many2one('res.users', string='Current Operator')
    commit_datetime = fields.Datetime(string='Commit Datetime')
    applicant = fields.Many2one('res.users', string='Applicant')
    commited = fields.Boolean(string='Commited', compute='_get_status', default=False)
    checked = fields.Boolean(string='Checked', compute='_get_status', default=False)
    approved = fields.Boolean(string='Approved', compute='_get_status', default=False)
    duration = fields.Integer(string='Duration')
    float = fields.Integer(string='Float', compute='_get_float')
    checked_human = fields.Many2one('res.users')
    approved_human = fields.Many2one('res.users')
    approveinfo_ids = fields.One2many('approve.info', 'task_stage_id')
    remarks = fields.Text(string='Remarks')
    state = fields.Selection(
        [('draft', 'Draft'), ('checked', 'Checked'), ('approved', 'Approved'), ('done', 'Done')], string='State', default='draft')

    @api.depends('actual_date', 'planned_date')
    def _get_float(self):
        for r in self:
            if r.actual_date and r.planned_date:
                planned_date = r.planned_date
                planned_date = planned_date.split('-')
                actual_date = r.actual_date
                actual_date = actual_date.split('-')

                d1 = datetime.date(int(planned_date[0]), int(planned_date[1]), int(planned_date[2]))
                d2 = datetime.date(int(actual_date[0]), int(actual_date[1]), int(actual_date[2]))
                days = rrule.rrule(rrule.DAILY, dtstart=d1, until=d2).count()
                r.float = days
...
class ApproveInfo(models.Model):
    """审批信息模型"""
    _name = 'approve.info'
    task_stage_id = fields.Many2one('task.stage')

    title = fields.Char(string='标题')
    remarks = fields.Text(string='意见')
    addperson = fields.Many2one('hr.employee', string='审批人')
    adddate = fields.Datetime(string='处理时间')
    status = fields.Selection([('0', u'同意'), ('1', u'驳回')], string='结果')
