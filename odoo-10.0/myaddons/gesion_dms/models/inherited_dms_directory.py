...
class Directory(dms_base.DMSModel):
    ...
    organization_id = fields.Many2one('gesion.organization', string="Organization", required=True)
    ...
    FileReader_ids = fields.Many2many('res.users', 'read_user_ref', 'read_id', 'user_id', string='File Reader')
    FileEdit_ids = fields.Many2many('res.users', 'edit_user_ref', 'edit_id', 'user_id', string='File Edit')
    FileDownLoad_ids = fields.Many2many('res.users', 'down_load_user_ref', 'down_load_id', 'user_id', string='File DownLoad')
    FileAdd_ids = fields.Many2many('res.users', 'add_user_ref', 'add_id', 'user_id', string='File Create')

    canceled = fields.Boolean(string="Canceled")

    def toggle_canceled(self):
        self.update({'canceled': not self.canceled})
